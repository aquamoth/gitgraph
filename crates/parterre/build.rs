...
fn main() {
    ...
    windows_icon(&icon_dir.join("parterre.ico"));
}

/// Embeds `ico` (`packaging/icon/parterre.ico`, generated by
/// `cargo run --release -p parterre-core --example icon_assets`) in the Windows executable,
/// through a resource script written here. Cosmetic, so a missing resource compiler only warns:
/// `rc.exe` comes with the Windows SDK, and checking the `windows-gnu` target from Linux needs
/// `x86_64-w64-mingw32-windres`. On other targets this does nothing.
fn windows_icon(ico: &Path) {
